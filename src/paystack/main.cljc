(ns paystack.main
  "Kotodama WASM entrypoint for the Paystack clean-room actor (L5) — Clojure port.
  ...
  facts (`paystack.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "paystack")
(def tier "L5")
...
(def entity-specs
  [{:entity "Transaction" :plural "transactions" :id-prefix "paystack_tra"
    :fields [:email :amount :reference :callbackUrl :plan :invoiceLimit :splitCode :subaccount :transactionCharge :bearer :label]
    :required [:email :amount]
    :coerce {:amount :int :invoiceLimit :int} :refs {}}
   {:entity "Customer" :plural "customers" :id-prefix "paystack_cus"
    :fields [:email :firstName :lastName :phone :metadata]
    :required [:email :firstName]
    :coerce {} :refs {}}
   {:entity "Plan" :plural "plans" :id-prefix "paystack_pla"
    :fields [:name :amount :interval :description :sendInvoices :sendSms :currency :invoiceLimit]
    :required [:name :amount]
    :coerce {:amount :int :invoiceLimit :int :sendInvoices :bool :sendSms :bool} :refs {}}
   {:entity "Subscription" :plural "subscriptions" :id-prefix "paystack_sub"
    :fields [:customer :plan :authorization :startDate]
    :required [:customer :plan]
    :coerce {} :refs {}}
   {:entity "Product" :plural "products" :id-prefix "paystack_pro"
    :fields [:name :description :price :currency :unlimited :quantity :splitCode :metadata]
    :required [:name :description]
    :coerce {:price :int :quantity :int :unlimited :bool} :refs {}}
   {:entity "Page" :plural "pages" :id-prefix "paystack_pag"
    :fields [:name :description :amount :currency :slug :type :plan :fixedAmount :splitCode :redirectUrl :successMessage :notificationEmail :collectPhone]
    :required [:name :description]
    :coerce {:amount :int :fixedAmount :bool :collectPhone :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"paystack.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "paystack-compat" :tier tier :entities entities} 200])
...
(defn start! [] :paystack-compat/ready)
